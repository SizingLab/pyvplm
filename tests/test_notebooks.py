...
from pyvplm.addon.variablepowerlaw import buckingham_theorem, automatic_buckingham, reduce_parameter_set, \
    declare_func_x_to_pi, regression_models, perform_regression, force_buckingham, pi_sensitivity, pi_dependency
...
def test_notebook1():
    ...
    lambda_wind = PositiveParameter('lambda_wind', [.1, 3], 'W/(m*K)',
                                    'equivalent thermal conductivity for winding')
    ...
    doeX, _ = create_const_doe(reduced_parameter_set, reduced_pi_set, func_x_to_pi, whished_size=50, test_mode=True)
    plot.close('all')
    ...
    doeX_FEM = import_file('notebooks/01_inductor_example/output_01', parameter_set, units='from_file')
...
def test_notebook2():
    ...
    models3 = regression_models(doePI.values, elected_pi0='pi1', order=3, log_space=False, test_mode=True)
    ...
    pi3 = df2_reduced['J']
    pi4 = float(RHO) / float(BETA) * (0.0254 * df2_reduced['DIAMETER (IN)']) ** 2 * (
    ...
    models5 = regression_models(doePI2.values, elected_pi0='pi1', order=2, log_space=False, test_mode=True)
...
def test_notebook_4():
    ...
    parameter_set = PositiveParameterSet(d_e, d_i, e_tooth, e_yoke, w_pm, r_i, j, B_R, B_SAT, MU_0, RHO_WIND,
                                         s_wind, p_jl)
